...
function createBayes(cfg) {
    ...
    function completeStep(rule, f, doneAt, fired, resolveState) {
        ...
        // A level check is answered on the spot: the condition either holds at the
        // moment the previous step completed, or the sequence does not apply.
        if (next.pattern === 'is') {
            const raw = resolveState ? resolveState(next) : undefined;
            if (conditionMatches(next, raw)) { completeStep(rule, f, doneAt, fired, resolveState); }
            else { resetFsm(rule.id); }
            return;
        }
