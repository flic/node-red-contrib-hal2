...
//
// A momentary rule holds ONE term by default: firing again restates it with a fresh
// clock rather than adding to it. Set repeat:'stack' on a rule whose repeated firings
// are independent observations and should accumulate.
//
...
const { CONVERTERS, COMPARE, rangeBounds } = require('./rules');
...
function createBayes(cfg) {
    ...
    function conditionMatches(step, raw, resolveState) {
        ...
        try {
            let b;
            // A range compares against a pair of bounds. The converters take one value each, so
            // the pair is assembled here rather than pretending to be a value type — the same
            // shape hal2Event and hal2Gate use.
            if (step.operator === 'range' || step.operator === 'outrange') {
                b = rangeBounds(step.value, step.valueHigh);
            } else if (step.valueType === 'state') {
    ...
    function fire(rule, now, value) {
        ...
        // How a rule's own repeated firings combine. 'refresh' (the default) keeps one term per
        // rule: firing again restates the same claim with a fresh clock, it does not make the
        // claim stronger. 'stack' adds a second term, which is the correct Bayesian reading only
        // when the two firings are genuinely independent observations.
        //
        // Refresh is the default because the common case is not independent. A motion sensor
        // re-triggering every few seconds is one continuing fact reported many times, and
        // stacking it drove a live node to the clamp on seventeen firings in twenty-three
        // minutes. Independence has to be claimed deliberately, not assumed.
        if (rule.repeat !== 'stack') {
            S.terms = S.terms.filter(t => t.ruleId !== rule.id);
        }
    ...
    function driveFsm(rule, s, isRising, isFalling, now, fired, resolveState) {
        ...
        } else if (isRising) { // 'becomes' and 'held'
            if (s === 0) { fsm.armedAt = now; }
            // The edge satisfies 'becomes'. It does not satisfy 'held' — that a condition has
            // turned true is precisely not the claim 'held' makes, and completing here made the
            // hold duration do nothing whenever the edge arrived after the step was armed, which
            // is the ordinary case. handleEvent has already recorded the edge; the tick starts
            // from it and completes the step once the duration has really passed.
            if (step.pattern !== 'held') {
    ...
    return {
        ...
        evaluate(resolveState, now) {
            ...
            for (const rule of cfg.rules) {
                ...
                let holds = true;
                // The reading of whichever step failed. `value` above is always step 1's, so on
                // a multi-step rule the two describe different steps — reading the status off
                // step 1 said "condition-false" for a step that had nothing to read at all.
                let failedValue;
                for (let i = 0; i < rule.steps.length && holds; i++) {
                    ...
                    // Name the step that failed, and what it was reading: with several
                    // conditions, "which one, and on what" is the whole question.
                    if (!holds) {
                        failedValue = v;
                        if (rule.steps.length > 1) {
                            e.failedStep = i + 1;
                            if (v !== undefined) { e.failedValue = v; }
                        }
                    }
                }
                ...
                if (holds) {
                    ...
                    // Of the step that failed, not of step 1: nothing to read is a different
                    // situation from read it and it was false, and only the failing step knows.
                    e.status = failedValue === undefined ? 'no-value' : 'condition-false';
                }
            }
