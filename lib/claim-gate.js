...
// The scopes a token carries, as a list. OAuth defines `scope` as a space-delimited string
// (RFC 6749 3.3), but providers differ — some send an array, Entra names it `scp` — so both
// shapes are read. Deliberately NOT routed through grants(): that one splits a configured list
// on commas and compares a string claim whole, which is right for group names (they may contain
// spaces) and wrong for scopes. Keeping them apart is what makes the group behaviour provably
// unchanged by this axis existing.
function tokenScopes(claims, scopeClaim) {
    const v = readClaim(claims, scopeClaim || 'scope');
...
function scopeAllows(claims, scopeClaim, list) {
    const wanted = parseList(list);
    if (!wanted.length) return true;
    const held = tokenScopes(claims, scopeClaim);
...
function createToolGate({ claims, claimName, serverValue, scopeClaim, serverScope }) {
    const serverGranted = claimAllows(claims, claimName, serverValue)
                       && scopeAllows(claims, scopeClaim, serverScope);
    return {
        serverGranted,
        allows: (list, scopeList) => serverGranted
            && claimAllows(claims, claimName, list)
            && scopeAllows(claims, scopeClaim, scopeList)
    };
}
